<template>
  <v-app>
    ...
    <v-app-bar color="black" dark app>
      ...
      <v-btn text class="d-none d-sm-flex" @click="router.push({ name: 'loginModal' })">
  {{ t("nav.login-nav") }}
</v-btn>
      <v-btn-toggle
...
// watch route name → เปิด popup ถ้า route เป็น loginModal
watch(
  () => route.name,
  (name) => {
    loginDialog.value = name === 'loginModal'
  },
  { immediate: true }
)

// ปิด popup → กลับ route หน้าเดิม
function closeLogin() {
