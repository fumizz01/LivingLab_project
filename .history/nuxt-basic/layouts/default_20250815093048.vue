      ...
      <v-app-bar color="black" dark app>
        <div style="display: flex; align-items: center">
          <v-toolbar-title class="pl-5 pr-2 mr-2 font-weight-bold">
            <NuxtLink class="logo-link" :to="$localePath('index')"> Living Lab </NuxtLink>
          </v-toolbar-title>
          <div
            class="nav-left d-none d-sm-flex"
            style="display: flex; align-items: center"
          >
            <v-btn class="nav-btn" variant="text" :to="$localePath('index')">{{ t('nav.home') }}</v-btn>
            <v-btn class="nav-btn" variant="text" :to="$localePath('qrscan')">{{ t('nav.qrScan') }}</v-btn>
            <v-btn class="nav-btn" variant="text" :to="$localePath('explore')">{{ t('nav.explore') }}</v-btn>
          </div>
        </div>
          ...
          <v-card class="custom-search-card">
            <v-card-text>
              <div class="dialog-search-bar" style="margin: 8px 0">
                ...
                />
                <v-btn text :to="$localePath('explore')" @click="handleSearch">ค้นหา</v-btn>
              </div>
            </v-card-text>
          </v-card>
        </v-dialog>
        ...
        <v-menu class="d-sm-none" offset-y location="top end">
          ...
          <v-list style="width: 100%">
            <v-list-item :to="$localePath('index')">{{ t('nav.home') }}</v-list-item>
            <v-list-item :to="$localePath('qrscan')">{{ t('nav.qrScan') }}</v-list-item>
            <v-list-item :to="$localePath('explore')">{{ t('nav.explore') }}</v-list-item>
...
function handleSearch() {
  ...
  router.push({ name: 'explore', params: { locale: locale.value } });
