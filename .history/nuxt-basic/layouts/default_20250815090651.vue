  ...
  <v-app>
      ...
      <v-app-bar color="black" dark app>
        <div style="display: flex; align-items: center">
          ...
          >
            <v-btn class="nav-btn" variant="text" to="/">{{ t('nav.home') }}</v-btn>
            <v-btn class="nav-btn" variant="text" to="/qrscan">{{ t('nav.qrScan') }}</v-btn>
            <v-btn class="nav-btn" variant="text" to="/explore">{{ t('nav.explore') }}</v-btn>
          </div>
        </div>
        ...
        <v-menu class="d-sm-none" offset-y location="top end">
          ...
          <v-list style="width: 100%">
            ...
            <v-list-item to="/qrscan">{{ t('nav.qrScan') }}</v-list-item>
            <v-list-item to="/explore">{{ t('nav.explore') }}</v-list-item>
            <v-list-item to="/#">{{ t('nav.login') }}</v-list-item>
            <v-list-item>
              <v-btn-toggle
                class="lang-toggle"
                mandatory
...
function switchLang(code) {
  if (code === 'en') {
    locale.value = 'en';
    lang.value = 'EN';
    // Use Nuxt i18n localePath for correct navigation
    const path = `/en${route.fullPath.replace(/^\/en/, '')}`;
    if (route.fullPath !== path) router.push(path);
  } else {
    locale.value = 'th';
    lang.value = 'TH';
    // Remove /en prefix for Thai
